...
(function () {
  ...
  function сказать(повод, задержка) {
    ...
    setTimeout(() => {
      /* Проверяем ещё раз: за эти доли секунды игрок мог выключить голос или
         питомец мог уснуть. Реплика, вылетевшая после выключения, выглядит
         так, будто переключатель не работает. */
      if (!isOn()) return;
      try {
  ...
  /* Про событие надо рассказать не только вслух. Журнал и разговор живут в
     talk.js и подписываются сюда — так наблюдатель остаётся один на всех. */
  function сообщить(повод) {
    for (const fn of (window.TamaAIEvents || [])) {
      try { fn(повод); } catch (e) { console.warn('[ии] подписчик:', e); }
    }
  }
